...
import com.dangerfield.hiittimer.libraries.ui.components.text.removeUnknownTags
...
fun String.processHtmlTags(): AnnotatedString {
    ...
    return buildAnnotatedString {
        com.dangerfield.hiittimer.libraries.ui.components.text.recurse(cleanedString, this)
    }
}
...
private fun String.removeUnknownTags(): StringBuilder {
    ...
    Regex("<[^>]*>").findAll(this).forEach {
        if (it.value !in com.dangerfield.hiittimer.libraries.ui.components.text.knownTags.keys && it.value !in com.dangerfield.hiittimer.libraries.ui.components.text.knownTags.values) {
            removeRanges.add(it.range)
        }
    }

    // Build the string without the unwhiittimerd tags
...
private fun recurse(string: String, to: AnnotatedString.Builder) {
    ...
    val startTag = com.dangerfield.hiittimer.libraries.ui.components.text.knownTags.keys.find { string.startsWith(it) }

    //Find the closing tag that the given String starts with, if any.
    val endTag = com.dangerfield.hiittimer.libraries.ui.components.text.knownTags.values.find { string.startsWith(it) }

    when {
        //If the String starts with a closing tag, then pop the latest-applied
        //SpanStyle and continue recursing.
        com.dangerfield.hiittimer.libraries.ui.components.text.knownTags.any { string.startsWith(it.value) } -> {
            to.pop()
            com.dangerfield.hiittimer.libraries.ui.components.text.recurse(string.removeRange(0, endTag!!.length), to)
        }
        //If the String starts with an opening tag, apply the appropriate
        //SpanStyle and continue recursing.
        com.dangerfield.hiittimer.libraries.ui.components.text.knownTags.any { string.startsWith(it.key) } -> {
            to.pushStyle(com.dangerfield.hiittimer.libraries.ui.components.text.tagToStyle(startTag!!))
            com.dangerfield.hiittimer.libraries.ui.components.text.recurse(string.removeRange(0, startTag.length), to)
        }
        ...
        com.dangerfield.hiittimer.libraries.ui.components.text.knownTags.any { string.contains(it.key) || string.contains(it.value) } -> {
            val firstStart =
                com.dangerfield.hiittimer.libraries.ui.components.text.knownTags.keys.map { string.indexOf(it) }.filterNot { it == -1 }.minOrNull() ?: -1
            val firstEnd =
                com.dangerfield.hiittimer.libraries.ui.components.text.knownTags.values.map { string.indexOf(it) }.filterNot { it == -1 }.minOrNull()
                    ?: -1
            val first = when {
            ...
            com.dangerfield.hiittimer.libraries.ui.components.text.recurse(string.removeRange(0, first), to)
        }
