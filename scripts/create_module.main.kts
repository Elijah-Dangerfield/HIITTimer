...
fun main() {
    printBlue("🚀 HIIT Timer Module Creator")
    println()
...
fun checkForHelp(): Boolean {
    ...
    if (isHelpRequest) {
        printBlue("""
            HIIT Timer Module Creator
            
            This script creates new KMP modules for the HIIT Timer project with proper structure and configuration.
...
fun createMainSourceFile(config: ModuleConfig, kotlinDir: File) {
    val packageName = "com.dangerfield.hiittimer.${config.baseDir}.${config.fullName.replace(":", ".")}"
...
fun createFeatureEntryPoint(config: ModuleConfig, kotlinDir: File, packageName: String) {
    ...
    val (routePackage, routeClass) = if (config.moduleName == "impl" && config.parentModule != null) {
        ...
        val parentPackage = "com.dangerfield.hiittimer.${config.baseDir}.${config.parentModule}"
...
${routeImport}import com.dangerfield.hiittimer.libraries.navigation.FeatureEntryPoint
...
fun createBuildGradle(config: ModuleConfig, moduleDir: File) {
    ...
    val pluginId = when (config.type) {
        ModuleType.FEATURE -> "hiittimer.feature"
        ModuleType.LIBRARY -> "hiittimer.kotlin.multiplatform"
    }
    
    val namespace = "com.dangerfield.hiittimer.${config.baseDir}.${config.fullName.replace(":", ".")}"
